...
export class AddNewUserComponent implements OnInit {
    ...
    // get fPinUpdate(): { [key: string]: AbstractControl } {
    //     return this.updatePinForm.controls;
    // }
    ...
    getBasicInformationDetailsByEmpId() {
        ...
        this.adminLayoutService.getBasicDetailsByEmployeeID(empId).subscribe((Response: any) => {
            if (Response.meta.code == 200) {
                ...
                this.empBasicInfoList.EmpId = Response.data.EmpId;
            }
        });
    }
    // end
    ...
    removeDocument(index: any, action: any) {
    ...
    saveDocumentUpload() {
        ...
        documentUploadModelObj.append('documentNumber', this.documentUploadForm.value.documentNumber);
    ...
    deleteDocumentDetailsById(id: any) {
        let Obj = {
            _id: id
        }
    ...
    closePinUpdateModel() {
        ...
        this.resetPin.setValue(null);
    }
    onPINChange(pin: any) {
        this.pinReset = pin;
        if (this.pinReset.length == 4) {
            this.pinResetMessage = null;
        }
    }
    ...
    updateLoginPinOfUser() {
        ...
        if (this.pinReset == null || !this.pinReset || this.pinReset.length != 4) {
            return;
        }
        ...
        this.adminLayoutService.resetPinByEmployeeId(Obj).subscribe((Response: any) => {
            if (Response.meta.code == 200) {
                // this.commonService.notifier.notify('success', "Pin Updated Successfully.");
                this.closePinUpdateModel();
            }
            else {
                // this.commonService.notifier.notify('error', Response.meta.message);
            }
    ...
    getCompanyInformationById(id: any) {
        ...
        this.adminLayoutService.getCompanyDetailsByID(EducationDetailsObj).subscribe((Res: any) => {
            if (Res.meta.code == 200) {
                ...
                this.companyInfoForm.controls['c_1stSalRaise'].setValue(Res.data.firstSalRaise);
    ...
    getCompanyActiveList() {
        ...
        this.adminLayoutService.getCompanyActiveList().subscribe((Response: any) => {
    ...
    getSalaryDetailsByEmployeeID() {
        ...
        this.salaryDetailsList.forEach((employee: any) => {
            const partsS = employee.startDate.split('-');
            const formattedStDate = `${partsS[2]}-${partsS[1]}-${partsS[0]}`;
            employee.startDate = formattedStDate;
            const partsE = employee.startDate.split('-');
            const formattedEnDate = `${partsE[2]}-${partsE[1]}-${partsE[0]}`;
            employee.endDate = formattedEnDate;
        });
        this.adminLayoutService.getSalaryDetailsByEmployeeID(Obj).subscribe((Response: any) => {
            if (Response.meta.code == 200) {
                ...
                this.salaryDetailsList.forEach((employee: any) => {
                    ...
                    const partsS = employee.startDate.split('/');
                    const formattedStartDate = `${partsS[2]}-${partsS[1]}-${partsS[0]}`;
                    const partsE = employee.endDate.split('/');
                    const formattedEndDate = `${partsE[2]}-${partsE[1]}-${partsE[0]}`;
                    employee.startDate = formattedStartDate;
                    employee.endDate = formattedEndDate;
                    console.log(employee.endDate);
                });
            }
        });
    ...
    getMonthlySalaryDetailsById(id: any) {
        let SalaryDetailsObj = {
            _id: id
        }
    ...
    createLeaveBalance(oItem?: any): FormGroup {
    ...
    getLeaveTypeList() {
        ...
        this.adminLayoutService.getLeaveBalanceByEmployeeID(Obj).subscribe((Response: any) => {
            if (Response.meta.code == 200) {
                ...
                casualArray = casualArray.sort((a: any, b: any) => months[a.month] - months[b.month]);
                sickArray = sickArray.sort((a: any, b: any) => months[a.month] - months[b.month]);
                emergencyArray = emergencyArray.sort((a: any, b: any) => months[a.month] - months[b.month]);
    ...
    downloadExperianceLetter() {
        let downloadOfferLetter = {
            "employeeId": this.userId,
        };


        this.adminLayoutService.downloadEmployeeExperiance(downloadOfferLetter).subscribe((Response: any) => {
            ...
            if (Response.meta.code == 200) {
                ...
                document.body.appendChild(downloadLink);
                downloadLink.href = url;
                var extension = "Experiance Letter.pdf";
                downloadLink.download = Response.data.FullName + ' - ' + extension;
                downloadLink.target = '_blank';
                downloadLink.click();
