...
export class ScrollBarDirective implements AfterViewInit {
  ...
  constructor(private elementRef: ElementRef) { }
    ...
    $("#" + id).mCustomScrollbar({
      ...
      callbacks: {
        onScroll: function () {
          const mcsInstance = $("#" + id).data("mCustomScrollbar");
          if (mcsInstance?.topPct > self.scrollTrigger && self.isScroll === true) {
            self.callScrollPaginationMethod.next(null);
          }
        }
      }
    });
...
export class DatepickertDirective implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    var options = {
      ...
      onChangeMonthYear: function (year: any, month: number, inst: any) {
    ...
    $(".datepicker").datepicker(options).on('changeDate',  (ev: any) =>{
      var curDate = $(this).datepicker("getDate");
      //curDate.setHours(0);
  ...
  onWindowScroll() {
    ...
    if (scrollPercent > this.scrollTrigger) {
      if (this.isScroll === true) {
        this.callScrollPaginationMethod.next(null);
      }
    }
  }
}
...
export class ConfirmDirective {
  ...
  @HostListener('click', ['$event']) clicked(e: any) {
      console.log("ConfirmDirective clicked");
